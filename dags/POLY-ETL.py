...
with DAG(
    dag_id="poly-etl",
    schedule=None if poly_etl_schedule == "None" else poly_etl_schedule,
   ...
   # params={"example_key": "example_value"},
    ...
    etl_job_task = KubernetesJobOperator(
        task_id='poly_file_upload',
        job_template_file='{{var.value.poly_job}}',
    )
    ...
    start_poly_extract_1 = EmptyOperator(
        task_id="Start_poly_Extract_1",
    )

    start_poly_extract_2 = EmptyOperator(
        task_id="Start_poly_Extract_2",
    )

    wait_time_task = HttpOperator(
        task_id='Wait_Time_Report',
        method='POST',
        endpoint='{{var.value.poly_wait_time_url}}',
        response_check=lambda response: response.json()["statusCode"]==200,
        data='{"version" : "", "startDate" : "2025-05-05", "endDate":"", "updatedMinDate":"", "updatedMaxDate":"", "draft":false, "deleted":false, "status":"COMPLETED", "healthAuthority":"", "isHeaderAdded": true}',
        headers={"Content-Type": "application/json"},
    )

    check_poly_folder_task = KubernetesJobOperator(
        task_id='Check_poly_Shared_Folder',
        job_template_file='{{var.value.poly_emtydir_job}}',
        wait_until_job_complete=True,
    )

    check_poly_sftp_folder_task = KubernetesJobOperator(
        task_id='Check_poly_SFTP_Folder',
        job_template_file='{{var.value.poly_emtysftp_job}}',
        wait_until_job_complete=True,
    )

    check_poly_sftp_folder_task >> check_poly_folder_task >> start_poly_extract_1
    start_poly_extract_1 >> wait_time_task >>  etl_job_task
